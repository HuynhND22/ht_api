...
const repository = data_source_1.AppDataSource.getRepository(book_entity_1.Book);
    ...
    try {
        const storage = multer.diskStorage({
            ...
            destination: function (req, file, cb) {
                if (!fs_1.default.existsSync(`./public/uploads/books/`)) {
                    fs_1.default.mkdirSync(`./public/uploads/books/`, { recursive: true });
                }
                ...
                if (err) {
                    ...
                    try {
                        ...
                        if ((_a = req.file) === null || _a === void 0 ? void 0 : _a.filename)
                            file = 'uploads/books/' + ((_b = req.file) === null || _b === void 0 ? void 0 : _b.filename);
                        const saveData = {
                            name: book.name,
                        ...
                        const success = yield repository.findOne({ where: { bookId: result.bookId }, relations: ['category'] });
            ...
            return __awaiter(this, void 0, void 0, function* () {
                ...
                if (err) {
                    ...
                    try {
                        ...
                        const found = yield repository.findOne({ where: { bookId: bookId } });
                        if (!found)
...
const softDelete = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const bookId = parseInt(req.params.id);
        console.log(bookId);
        const found = yield repository.findOneBy({ bookId: bookId });
        if (!found)
            return res.status(410).json('Product not found');
        yield repository.softDelete({ bookId: bookId });
        res.sendStatus(200);
    }
    catch (error) {
        console.error(error);
...
const restore = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const found = yield repository.findOne({ where: { bookId: bookId, deletedAt: (0, typeorm_1.Not)((0, typeorm_1.IsNull)()) }, withDeleted: true, });
        if (!found)
            return res.status(410).json('Product not found');
        yield repository.restore({ bookId: bookId });
        res.sendStatus(200);
    }
    catch (error) {
        console.error(error);
...
const hardDelete = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const bookId = parseInt(req.params.id);
        const product = yield repository.findOne({ withDeleted: true, where: { bookId: bookId, deletedAt: (0, typeorm_1.Not)((0, typeorm_1.IsNull)()) } });
        if (!product)
            return res.sendStatus(410);
        yield repository.delete({ bookId: bookId });
        res.sendStatus(200);
    }
    catch (error) {
        console.error(error);
...
const checkProductUnique = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        const check = yield (0, checkUnique_1.default)(book_entity_1.Book, `${field}`, value);
...
const getByCategory = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const products = yield repository.find({ where: Object.assign({ categoryId: parseInt(categoryId) }, searchCondition), relations: ['images', 'category', 'supplier', 'status', 'productSizes.size'], order: { createdAt: 'DESC' } });
        if (products.length === 0) {
            return res.status(204).send({
                error: "No content",
            });
...
const client = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const search = req.query.search;
        const searchCondition = search ? { name: (0, typeorm_1.Like)(`%${search}%`) } : {};
        const products = yield repository.find({ where: Object.assign({}, searchCondition), relations: ['images', 'category', 'supplier', 'status', 'productSizes.size'], order: { createdAt: 'DESC' } });
        if (products.length === 0) {
            return res.status(204).send({
                error: "No content",
            });
